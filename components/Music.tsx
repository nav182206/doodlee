...
const Music: React.FC<MusicProps> = ({ editMode, onSave }) => {
  ...
  const handleAddSong = () => {
    ...
    const newSong: Song = {
      ...
      cover: `https://picsum.photos/seed/${editingIdx}/400/400`,
      spotifyUrl: embed
    };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-12 pb-32 px-4 md:px-0">
          ...
          <div className="relative group">
            ...
            <div className="bg-white rounded-[4rem] p-6 shadow-2xl border-4 border-white overflow-hidden transition-all duration-500 hover:shadow-green-100/50">
              {activeSong ? (
                ...
                <div className="h-[380px] flex flex-col items-center justify-center bg-gray-50 rounded-[3rem] border-4 border-dashed border-gray-100">
                  <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center text-4xl shadow-lg mb-6">🎧</div>
                  <p className="font-serif-elegant italic text-2xl text-gray-400">
                    {editMode ? "Add a link on the right" : "Pick a track to start the music"}
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="lg:w-1/3">
          <div className="bg-white/40 backdrop-blur-2xl rounded-[4rem] p-8 border border-white shadow-2xl sticky top-8">
            <div className="flex items-center justify-between mb-8 px-2">
               <h4 className="text-xs font-black uppercase tracking-[0.4em] text-gray-400">Memory Lane Playlist</h4>
               <div className="w-2 h-2 bg-green-400 rounded-full animate-ping"></div>
            </div>

            <div className="grid grid-cols-1 gap-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {songs.map((song, idx) => {
                if (!song && !editMode) return null; // Hide empty slots for her
                return (
                  <div key={idx} className="relative group">
                    <button
                      onClick={() => song ? setActiveIdx(idx) : editMode && setEditingIdx(idx)}
                    ...
                    >
                      ...
                      <div className="text-left flex-grow truncate">
                        ...
                        <div className={`text-[9px] font-black uppercase tracking-widest ${activeIdx === idx ? 'text-green-100' : 'text-green-400'}`}>
                          {song ? "PLAY NOW" : editMode ? "Add Spotify Link" : ""}
                        </div>
                      </div>
                    </button>
                    ...
                    {editMode && song && (
                      <div className="absolute right-3 top-1/2 -translate-y-1/2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                         <button onClick={(e) => { e.stopPropagation(); setEditingIdx(idx); }} className="w-8 h-8 bg-white text-blue-500 rounded-full flex items-center justify-center shadow-lg hover:bg-blue-50">✏️</button>
                         <button onClick={(e) => { e.stopPropagation(); removeTrack(idx); }} className="w-8 h-8 bg-white text-rose-500 rounded-full flex items-center justify-center shadow-lg hover:bg-rose-50">🗑️</button>
                      </div>
                    )}
                  </div>
                );
      ...
      {editMode && editingIdx !== null && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-green-900/20 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setEditingIdx(null)}>
          <div className="w-full max-w-md bg-white rounded-[3.5rem] p-12 shadow-2xl border border-green-50" onClick={e => e.stopPropagation()}>
            <div className="text-center space-y-4 mb-8">
               <h3 className="text-3xl font-serif-elegant font-bold text-gray-900">Add Track #{editingIdx + 1}</h3>
               <p className="text-gray-400 italic text-sm">Paste Spotify URL</p>
            </div>
            <div className="space-y-6">
              <input autoFocus type="text" value={tempUrl} onChange={(e) => setTempUrl(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddSong()} placeholder="https://open.spotify.com/track/..." className="w-full px-8 py-5 rounded-[2rem] bg-gray-50 border-2 border-transparent focus:border-green-400 focus:bg-white outline-none transition-all font-medium text-gray-600" />
              {error && <p className="text-rose-500 text-xs font-bold text-center animate-bounce">{error}</p>}
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => setEditingIdx(null)} className="py-5 rounded-[2rem] font-bold text-gray-400 hover:bg-gray-50">Cancel</button>
                <button onClick={handleAddSong} className="py-5 bg-green-500 text-white rounded-[2rem] font-bold shadow-xl hover:bg-green-600">Save Track</button>
              </div>
            </div>
          </div>
        </div>
